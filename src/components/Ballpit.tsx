  ...
  #initRenderer() {
    if (this.#config.canvas) {
      ...
      if (elem instanceof HTMLCanvasElement) {
        ...
        console.error('Three: Missing canvas orparam id eter');
      }
    } else {
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof DefaultXConfig> = {}) {
  ...
  #setupLights() {
    ...
    this.light = new PointLight(this.config.colors[1], this.config.lightIntensity);
    this.add(this.light);
  }

  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 10) {
